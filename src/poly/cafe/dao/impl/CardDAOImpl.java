...
public class CardDAOImpl implements CardDAO { 
    ...
    @Override
    public boolean isCardInUse(Integer id) {
        String sql = "SELECT COUNT(*) AS total FROM Bills WHERE CardId=? AND Status=0";
        try {
            java.sql.ResultSet rs = XJdbc.executeQuery(sql, id);
            if (rs.next()) {
