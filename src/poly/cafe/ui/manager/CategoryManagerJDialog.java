...
public class CategoryManagerJDialog extends javax.swing.JDialog implements CategoryController{
    ...
    public boolean validateForm() {
        ...
        // Kiểm tra độ dài và ký tự đặc biệt cho mã loại
        if (id.length() > 10) {
            XDialog.alert("Mã loại không được quá 10 ký tự!");
            txtId.requestFocus();
            return false;
        }
        
        if (!id.matches("^[a-zA-Z0-9]+$")) {
            XDialog.alert("Mã loại chỉ được chứa chữ và số!");
            txtId.requestFocus();
            return false;
        }
        
        // Kiểm tra độ dài tên loại
        if (name.length() > 50) {
            XDialog.alert("Tên loại không được quá 50 ký tự!");
            txtName.requestFocus();
            return false;
        }
    ...
    public void fillToTable() { 
        ...
        try {
            ...
            btnMoveFirst.setEnabled(hasData);
            btnMovePrevious.setEnabled(hasData); 
            btnMoveNext.setEnabled(hasData);
            btnMoveLast.setEnabled(hasData);
    ...
    public void deleteCheckedItems() { 
        ...
        if (XDialog.confirm("Bạn có chắc muốn xóa " + count + " mục đã chọn?")) {
            try {
                int successCount = 0;
                for (int i = 0; i < tblCategories.getRowCount(); i++) {
                    if ((Boolean) tblCategories.getValueAt(i, 2)) {
                ...
                XDialog.alert("Lỗi xóa dữ liệu: " + e.getMessage());
            }
        } 
    } 
    ...
    public void update() { 
        try {
            if (validateForm()) {
                if (XDialog.confirm("Bạn có muốn cập nhật bản ghi này?")) {
                    Category entity = this.getForm();
                    dao.update(entity);
                    this.fillToTable();
    ...
    public void setEditable(boolean editable) {
        // Quản lý trạng thái các controls
        ...
        boolean hasData = tblCategories.getRowCount() > 0;
